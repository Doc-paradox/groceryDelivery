...
const StyledCard = styled(Card)(({ theme }) => ({
  ...
  transition: 'all 0.3s ease-in-out',
  boxShadow: theme.shadows[3],
  position: 'relative', // Ensure the card is relative for absolute positioning of children
  '&:hover': {
    transform: 'translateY(-5px)',
    boxShadow: theme.shadows[8],
  },
          ...
          {productlist.map((product) => (
            <StyledCard key={product.productid}>
              {editingProductId === product.productid ? (
                ...
                <CardContent>
                  <EditButton onClick={() => setEditingProductId(product.productid)}>
                    <Edit />
                  </EditButton>
                  <Typography variant="h5" gutterBottom>
                    Product #{product.productid}
                  ...
                  <Typography variant="body1">Category: {product.category}</Typography>
                  <Typography variant="body1">Manufacture Date: {product.manufactutredate}</Typography>
                  <Typography variant="body1" gutterBottom >Expiry Date: {product.expirydate}</Typography>
                  <DeleteButton onClick={() => handleDelete(product.productid)}>
                    DELETE
                  </DeleteButton>
                </CardContent>
              )}
